...
const rules = {
  ...
  messageDelay: { required, minValue: minValue(0) },
};
...
const formErrors = computed(() => ({
  ...
  messageDelay: getErrorMessage('messageDelay', 'MESSAGE_DELAY'),
}));
...
const prepareCampaignDetails = () => ({
  ...
  additional_attributes: {
    message_delay: Number(state.messageDelay),
  },
});
...
<template>
  <form class="flex flex-col gap-4" @submit.prevent="handleSubmit">
    ...
    <Input
      v-model="state.messageDelay"
      :label="t('CAMPAIGN.API.CREATE.FORM.MESSAGE_DELAY.LABEL')"
      type="number"
      min="0"
      step="0.5"
      :placeholder="t('CAMPAIGN.API.CREATE.FORM.MESSAGE_DELAY.PLACEHOLDER')"
      :message="
        formErrors.messageDelay ||
        t('CAMPAIGN.API.CREATE.FORM.MESSAGE_DELAY.INFO')
      "
      :message-type="formErrors.messageDelay ? 'error' : 'info'"
    />
