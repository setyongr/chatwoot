...
const addCampaign = async campaignDetails => {
  try {
    ...
    useAlert(t('CAMPAIGN.API_CHANNEL.CREATE.FORM.API.SUCCESS_MESSAGE'));
  } catch (error) {
    const errorMessage =
      error?.response?.message ||
      t('CAMPAIGN.API_CHANNEL.CREATE.FORM.API.ERROR_MESSAGE');
...
<template>
  ...
  >
    <h3 class="text-base font-medium text-n-slate-12">
      {{ t('CAMPAIGN.API_CHANNEL.CREATE.TITLE') }}
    </h3>
